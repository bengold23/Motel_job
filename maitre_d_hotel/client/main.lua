...
Citizen.CreateThread(function()
	...
	if ESX.PlayerData.job.name == 'maitrehotel' then
...
function OpenmaitrehotelMenu()
	local elements = {
		{label = _U('hotels'), value = 'hotels'},
		{label = _U('clients'),    value = 'customers'},
	}

	if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'maitrehotel' and ESX.PlayerData.job.grade_name == 'boss' then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'maitrehotel', {
		title    = _U('realtor'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)

		if data.current.value == 'hotels' then
			OpenhotelMenu()
		elseif data.current.value == 'customers' then
			OpenCustomersMenu()
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'maitrehotel', function(data, menu)
				menu.close()
			end)
		end
		...
		CurrentAction     = 'maitrehotel_menu'
...
function OpenhotelMenu()
	TriggerEvent('ilyes_hotel:gethotels', function(hotels)

		local elements = {
			head = {_U('hotel_name'), _U('hotel_actions')},
			rows = {}
		}
		...
		for i=1, #hotels, 1 do
			table.insert(elements.rows, {
				...
				cols = {
					hotels[i].label,
					_U('hotel_actionbuttons')
				}
			})
		end

		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'hotels', elements, function(data, menu)

			if data.value == 'sell' then

				menu.close()

				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell_hotel_amount', {
					title = _U('amount')
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							TriggerServerEvent('maitre_d_hoteljob:sell', GetPlayerServerId(closestPlayer), data.data.name, amount)
							menu2.close()
						end
			...
			elseif data.value == 'rent' then
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'rent_hotel_amount', {
					title = _U('amount')
				}, function(data2, menu2)
					local amount = tonumber(data2.value)
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							TriggerServerEvent('maitre_d_hoteljob:rent', GetPlayerServerId(closestPlayer), data.data.name, amount)
							menu2.close()
						end
			...
			elseif data.value == 'gps' then
				...
				TriggerEvent('ilyes_hotel:gethotel', data.data.name, function(hotel)
					if hotel.isSingle then
						SetNewWaypoint(hotel.entering.x, hotel.entering.y)
					else
						TriggerEvent('ilyes_hotel:getGateway', hotel, function(gateway)
...
function OpenCustomersMenu()
	ESX.TriggerServerCallback('maitre_d_hoteljob:getCustomers', function(customers)
		local elements = {
			head = {_U('customer_client'), _U('customer_hotel'), _U('customer_agreement'), _U('customer_actions')},
			rows = {}
		}
		...
		for i=1, #customers, 1 do
			table.insert(elements.rows, {
				...
				cols = {
					customers[i].name,
					customers[i].hotelLabel,
					(customers[i].hotelRented and _U('customer_rent') or _U('customer_sell')),
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'customers', elements, function(data, menu)
			...
			if data.value == 'revoke' then
				TriggerServerEvent('maitre_d_hoteljob:revoke', data.data.hotelName, data.data.hotelOwner)
				OpenCustomersMenu()
			elseif data.value == 'gps' then
				TriggerEvent('ilyes_hotel:gethotel', data.data.hotelName, function(hotel)
					if hotel.isSingle then
						SetNewWaypoint(hotel.entering.x, hotel.entering.y)
					else
						TriggerEvent('ilyes_hotel:getGateway', hotel, function(gateway)
...
AddEventHandler('maitre_d_hoteljob:hasEnteredMarker', function(zone)
	...
	elseif zone == 'OfficeActions' and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'maitrehotel' then
		CurrentAction     = 'maitrehotel_menu'
		CurrentActionMsg  = _U('press_to_access')
		CurrentActionData = {}
	end
...
AddEventHandler('maitre_d_hoteljob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('maitre_d_hoteljob:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('maitre_d_hoteljob:hasExitedMarker', LastZone)
		end
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				if CurrentAction == 'maitrehotel_menu' then
					OpenmaitrehotelMenu()
				end
